...
public class IndexingService {
  ...
  private void doIndex(UUID repoId, UUID userId) {
    ...
    for (String path : filePaths) {
      try {
        String content =
            githubApiClient.getFileContent(token, repo.getOwner(), repo.getName(), path);
      ...
      if (processed % PROGRESS_EVERY_N_FILES == 0 || processed == filePaths.size()) {
        updateProgress(
            repoId, filePaths.size(), processed, totalChunks, IndexStatus.INDEXING, null);
      }
      rateLimiter.pause();
    }
  ...
  protected void updateProgress(
      UUID repoId, int total, int processed, int chunks, IndexStatus status, String error) {
    repositoryRepository
        .findById(repoId)
        .ifPresent(
            repo -> {
  ...
  protected void markReady(
      UUID repoId, int totalFiles, int processedFiles, int totalChunks, String fullName) {
    repositoryRepository
        .findById(repoId)
        .ifPresent(
            repo -> {
